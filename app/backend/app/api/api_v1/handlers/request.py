...
@request_router.post('/', summary="Save offer details from URL", response_model=DataOut)
async def get_data_from_url(url: str, current_user: User = Depends(get_current_user)):
    request_service = RequestService()
    site, job_position, company_name, offer_url = request_service.data_from_url(url)
